...
# boost_inc_dir = os.path.join(os.environ["BOOST"], 'include')
yaml_inc_dir = os.path.join(os.environ["YAML_CPP"], 'include')
...
if os.environ.get('EASYMAKE_VERBOSE') or os.environ.get('VERBOSE'):
    compile_args.extend(['-v'])
    link_args.extend([ '-v' ])

# All Modules which can be built
PYTHON_STATIC_LIBS = get_config_vars()['LIBDIR']
EXTRA_OBJECTS = (os.path.join(PYTHON_STATIC_LIBS, '/libpython3.6.a'),)
