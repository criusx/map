...
class BaseWidget {
    constructor(element, id, default_height) {
        ...
        // Name of widget
        this.name = element.getAttribute('helios-widget-name')

        // Type of this helios widget
        this.typename = element.getAttribute('helios-widget-type')
        ...
        // Include a .helios-widget-sort-handle  so the widget can be sorted
        $(element).html(`
            <div style="display:grid; width:100%; height:100%; grid-template-rows:20px 1fr; grid-template-areas: 'header' 'body';">
                <div class="helios-widget-header" style="grid-area: header;">
                    ...
                    <span class="ui-icon ui-icon-arrowthick-2-n-s helios-widget-sort-handle"></span>
                    <div class="helios-widget-datasource circle-icon" style="top:2px;"><div class="icon-label">.</div></div>
                    <span class="helios-widget-title">${this.name}</span>
                    <span class="helios-widget-typename">(${this.typename})</span>
                    <span class="helios-widget-id">#${this.widget_id}</span>
                </div>
                <div class="helios-widget-body" style="grid-area: body;"></div>
            </div>
        ...
        this.jheader = $(element).find('.helios-widget-header')

        // Body jquery element
        this.jbody = $(element).find('.helios-widget-body')

        // Data-source icon
        this.jdatasource_icon = this.jheader.find('.helios-widget-datasource')
        ...
        const clone = $(`<div style="position:absolute; top:4px; right:100px" class="helios-collapse-button ui-icon ui-icon-newwin" title="Clone this widget"></div>`)
        clone.click(function() { self.clone() } )
        jbuttons.append(clone)
        const collapse = $(`<div style="position:absolute; top:4px; right:80px" class="helios-collapse-button ui-icon ui-icon-triangle-1-n" title="Collapse this widget to a single bar"></div>`)
        collapse.click(function() { self.collapse() } )
        jbuttons.append(collapse)
        const expand = $(`<div style="position:absolute; top:4px; right:60px" class="helios-expand-button ui-icon ui-icon-arrowthick-2-ne-sw" title="Expand this widget. If collapsed, expands back to previous size. If not collapsed, expands to default size."></div>`)
        expand.click(function() { self.expand() } )
        jbuttons.append(expand)
        const gear = $(`<div style="position:absolute; top:4px; right:40px" class="helios-settings-button ui-icon ui-icon-gear" title="open detailed widget settings"></div>`)
        gear.click(function() { self.settings() } )
        jbuttons.append(gear)
        const trash = $(`<div style="position:absolute; top:4px; right:20px" class="helios-close-button ui-icon ui-icon-trash" title="move widget to trash. It can be recovered from trash until this tab is closed or reloaded"></div>`)
    ...
    make_droppable() {
        ...
        $(this.element).droppable({
            ...
            classes: {
              ...
              "ui-droppable-hover": "helios-drop-hover" // feedback when hovering
            },
            drop: (event, ui) => {
    ...
    settings() {
        ...
        const dlg = $(`<div style="padding:8px; z-index: 2000; border-radius:8px; border: 1px solid black; background-color:white; min-width:200px; min-height:200px; position: fixed; left:50%; top:50%; transform: translate(-50%,-50%);
                        display:grid; grid-template-rows: auto 1fr auto; grid-template-areas: 'header' 'content' 'footer';">
                           <div style="grid-area: header;">
                                ...
                                <hr/>
                                <!--<div style="position:absolute; top:10px; right:8px; cursor: pointer;" class="helios-close-button ui-icon ui-icon-closethick" title="close these settings"></div>-->
                           </div>
        ...
        //const close = dlg.find('.helios-close-button')
    ...
    // TODO: We should have a collection of settings widgets stylized for helios.
    on_settings(container) {
        ...
        name.on('keyup', () => {
            ...
            $(this.element).attr('helios-widget-name', new_name)
            this.name = new_name
            this.jheader.find('.helios-widget-title').html(new_name)
        })
