...
const viewer = new function () {
    ...
    // How to create classes backing various widget. Lookup by helios-widget-type
    this.widget_factory = {
    ...
    this.on_load = async function() {
        try {
            ...
            //this.prefix = 'ws://cbatch67045:8002'
            ...
            $('#widget-area').sortable({
                ...
                items: 'div.helios-widget:not(.ui-state-disabled)',
                handle: '.helios-widget-sort-handle',
            })
    ...
    this.logout = function() {
        ...
        location.reload(true) // reload page without caching
    }

    this.download_layout = function() {
        const json = this._get_json_layout()
        download_to_client(json, `helios-layout-${Math.floor(performance.now())}.txt`, 'text/plain')
    }
    ...
    this._get_json_layout = function() {
        ...
        const widgets = $('#widget-area').find('.helios-widget')
    ...
    this._get_widget_config_dict = function(pwidget) {
        const name = pwidget.element.getAttribute('helios-widget-name')
        const typename = pwidget.element.getAttribute('helios-widget-type')
    ...
    // Create a helios widget from a factory and optional name
    this.add_new_widget_from_factory = function (factory, { widget_name = null, data_id = null, request_processor = true } = {}) {
        ...
        const jw = $(`<div class="helios-widget" helios-widget-name="${widget_name}" helios-widget-type="${widget_type}"></div>`)
        jw.insertBefore(widget_area_trailer)

        // Create helios widget object and attach to element
